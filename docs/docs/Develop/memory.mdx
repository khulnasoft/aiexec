...
For more information, see the database models in the [source code](https://gitlab.com/khulnasoft/aiexec/tree/main/src/backend/base/aiexec/services/database/models).
...
The `AIEXEC_DB_CONNECTION_SETTINGS` is a JSON configuration for database connection pool settings that allows you to fine-tune your database connection pool and timeout settings.
...
Aiexec officially supports only the default asynchronous, in-memory cache. Other backends are experimental and may change without notice.
...
|----------|------|---------|-------------|
| `AIEXEC_CACHE_TYPE` | String | `async` | Set the cache type for Aiexec's internal caching system. |
| `AIEXEC_LANGCHAIN_CACHE` | String | `InMemoryCache` | Set the cache type for Langchain's caching system. |
| `AIEXEC_REDIS_HOST` | String | `localhost` | Redis server hostname. |
| `AIEXEC_REDIS_PORT` | Integer | `6379` | Redis server port. |
| `AIEXEC_REDIS_DB` | Integer | `0` | Redis database number. |
| `AIEXEC_REDIS_CACHE_EXPIRE` | Integer | `3600` | Cache expiration time in seconds. |
| `AIEXEC_REDIS_PASSWORD` | String | Not set | Redis authentication password (optional). |
