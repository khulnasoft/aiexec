...
2. Set the following environment variables in your Aiexec `.env` file, replacing `LANGCHAIN_API_KEY` and `LANGSMITH_PROJECT_NAME` with your own values:
...
3. Restart Aiexec with your modified `.env` file or from the terminal where you set your environment variables:
    ...
    However, Aiexec can source environment variables from `.env` _and_ your terminal.
    For more information, see [Aiexec environment variables](/environment-variables).

4. Run a flow in Aiexec to generate some activity.
