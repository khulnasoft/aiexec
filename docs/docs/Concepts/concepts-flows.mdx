...
The database storage location can be customized using the `AIEXEC_CONFIG_DIR` and `AIEXEC_SAVE_DB_IN_CONFIG_DIR` environment variables. The config directory can be customized with the `AIEXEC_CONFIG_DIR` environment variable.

Aiexec logs are stored in the config directory specified in the `AIEXEC_CONFIG_DIR` environment variable.
